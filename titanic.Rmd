...
library(lattice)
library(doParallel)
tr_set <- read.csv(file = "data/train.csv")
test_set <- read.csv(file = "data/test.csv")
gender_subm <- read.csv(file = "data/gender_submission.csv")
...
df$Survived <- factor(x = df$Survived, levels = c(0, 1), labels = c("0", "1"))
...
```{r}
df <- df[!is.na(df$Age),]
tr_set <- tr_set[!is.na(tr_set$Age),]
test_set <- test_set[!is.na(test_set$Age),]
test_set[is.na(test_set$Age),]
```
